...
async function join() {
  ...
  const cusAudioTrack = await getCustomAudioTrack();
  // Publish the local video and audio tracks to the channel.
  await client.publish([canvasVideoTrack,cusAudioTrack]);
...
async function getCustomAudioTrack(){
  ...
  try{
    ...
    console.table(audioTrack);
    return AgoraRTC.createCustomAudioTrack({
      mediaStreamTrack: audioTrack,
      encoderConfig: {
        bitrate: 48, sampleRate: 44000, stereo: true
      }
    })
  }
  catch(err) {
