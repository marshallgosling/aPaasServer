...
async function getCustomScreenTrack(){
  let displaystream = null;
  try{
    displaystream = await navigator.mediaDevices.getDisplayMedia({preferCurrentTab:true, audio: true, video: {
      width: 1280,
      height: 720,
    ...
    const v = AgoraRTC.createCustomVideoTrack({
    ...
    const a = AgoraRTC.createCustomAudioTrack({
      mediaStreamTrack: audioTrack,
      encoderConfig: {
        bitrate: 128, sampleRate: 44000, stereo: true
      }
    })
...
async function join() {
  ...
  // join a channel and create local tracks, we can use Promise.all to run them concurrently
      [options.uid, screenTrack] = await Promise.all([
    // join the channel
    client.join(options.appid, options.channel, options.token || null, options.uid || null),
    getCustomScreenTrack()
  ...
  if(screenTrack instanceof Array){
    //console.table(screenTrack);
    console.log("multi screenTracks");
    localTracks.screenVideoTrack = screenTrack[0]
    localTracks.screenAudioTrack = screenTrack[1]
  }
  else{
    console.log("single screenTrack");
  ...
  if(localTracks.screenAudioTrack == null){
    ...
    await client.publish([localTracks.screenVideoTrack, localTracks.screenAudioTrack]);
  }
